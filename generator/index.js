...
module.exports = (api, options) => {
  api.extendPackage({
    ...
    scripts: {
      ...
      "template:deploy":
        "cd .template && git init && git add -A && git commit -m 'deploy' && git push -f https://github.com/egova-safety/vue-preset-admin-template.git master:master",
...
module.exports.hooks = (api, options) => {
  api.afterInvoke(() => {
    try {
      ...
      // 移动文件
      move(
        api.resolve(`src/pages/${isSubModel ? "sub" : "main"}/application/`),
        api.resolve("src")
      )
      move(
      ...
      isSubModel && move(api.resolve("src/pages/sub/views"), api.resolve("src"))
      !isSubModel &&
